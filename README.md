...
The included **RX3 Stem Studio** workflow can read your Rekordbox USB/SSD directly.

You do **not** need to export a Rekordbox XML file for this workflow.

1. Prepare or update your Rekordbox USB/SSD normally in Rekordbox.
2. Connect the prepared USB/SSD to your computer.
3. Open **RX3 Stem Studio** and go to the **Stems preparation** tab.
4. Select the Rekordbox USB/SSD.
5. Stem Studio reads the Rekordbox database directly from the USB/SSD, including `export.pdb`.
6. The playlists stored on the USB/SSD are detected and displayed.
7. Select the playlist you want to process **before starting separation**.
8. Select the separation model and quality, then start the separation.

This means you can add new tracks to a Rekordbox USB/SSD and prepare their stems without having to generate or update a separate Rekordbox XML file first.

Stem Studio is independent of the Rekordbox application and database installed on the computer. It uses the Rekordbox database already stored on the prepared USB/SSD.

The existing XML-based workflow remains available where supported, but XML is **not required** when using the direct USB/SSD workflow.

### BS-RoFormer EP17

Stem Studio also includes the **BS-RoFormer EP17** separation model:

`model_bs_roformer_ep_17_sdr_9.6568.ckpt`

EP17 is used to generate separate:

* **Vocals**
* **Bass**
* **Drums**

The EP17 pipeline is part of the preparation for an expanded RX3 stem workflow.

The current RX3 BIN workflow still supports **two-stem operation**. Work is in progress to extend the BIN generation pipeline to a future **four-stem RX3 workflow**.
...
2. Connect the prepared Rekordbox USB/SSD to your computer.

3. Open **RX3 Stem Studio** and go to the **Stems preparation** tab.

4. Select the Rekordbox USB/SSD. Stem Studio reads the Rekordbox database directly from the device, including `export.pdb`. It detects the playlists stored on the USB/SSD and displays them for selection.

5. Select the playlist you want to separate **before starting the separation**.
...
If that output folder was not your Rekordbox USB/SSD, move the complete `RX3_STEMS` folder to the root of the stick now.
...
No. Prepared and unprepared tracks can coexist on the same Rekordbox USB/SSD. Tracks without stem sidecars continue to work normally.
...
No. The original audio files are not modified. Generated stems are separate RX3 sidecar files stored in the `RX3_STEMS` folder:
...
Stem Studio reads the Rekordbox database directly from a prepared USB/SSD, so the computer does not need the Rekordbox application or a Rekordbox XML export for the direct USB/SSD workflow.
